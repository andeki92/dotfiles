...
# Where the pane is right now, by the herdr server's account.
live_pane() {
  pane="$(run_bounded herdr pane get "$HERDR_PANE_ID" 2>/dev/null)"
  live_ws="$(printf '%s' "$pane" | jq -r '.result.pane.workspace_id // empty' 2>/dev/null)"
  live_tab="$(printf '%s' "$pane" | jq -r '.result.pane.tab_id // empty' 2>/dev/null)"
  [ -n "$live_ws" ] && [ -n "$live_tab" ] || exit 0
}
